...
The backtest uses simple annualized APY (period return × 365/days) — see Lesson 22. Headline ±1% with 40bps slippage and 6bps/day funding. Funding rates can flip negative in bear markets (shorts pay instead of earn), and slippage doubles during high-vol events. Key finding: ±1% is profitable even with zero funding — fee income alone covers IL. The biggest risk is slippage, not funding. Real-time IL tracking via `reposition_il_usd` and `cumulative_il_usd` in cycles.db computes actual IL vs fee income per day.

### 20. Jupiter swap `outAmount` is an estimate — always measure actual token received
The convenience methods (`swapUsdcToTslax`, etc.) returned `quote.outAmount` as the output amount, but this is the pre-execution estimate from the route planner. The actual on-chain amount differs due to price movement between quote and execution, MEV/sandwich attacks, and route execution variance. Fix: Query output token ATA balance before and after `confirmTransaction`, compute `actualOutput = postBalance - preBalance`. This also means downstream code using the swap result (like `tslaxBalance += BigInt(swapResult.tslaxAmount)` in repositionLP) now uses real amounts instead of estimates. Track `swap_slippage_bps` in cycles.db to monitor if slippage exceeds the 40bps measured baseline. For TSLAx output (8 decimals), convert to USD via `amount / 1e8 * currentPrice`. For USDC output (6 decimals), convert via `amount / 1e6`. Token-2022 (TSLAx) requires `TOKEN_2022_PROGRAM_ID` for ATA resolution.

### 21. Measure slippage from live quotes before assuming — backtest assumptions can be wrong
The original backtest used 30bps slippage (a guess). Live Jupiter quote analysis revealed the real TSLAx/USDC bid-ask spread is ~40bps across all tested sizes ($100–$5K). Jupiter routing price impact is negligible (0.1–0.2 bps) — the cost is almost entirely the pool spread. Always validate backtest assumptions with live market data before trusting results.

### 22. Don't use compound annualization for sub-year backtests
GIPS (Global Investment Performance Standards) recommends against annualizing returns for periods < 1 year. Using `g^(365/31)` on a 31-day backtest has an exponent of 11.66×, which amplifies tiny P&L differences into massive APY swings — a $137 cost increase (10bps × 65 repos) became a 16pp "APY" swing. The actual period return only changed from 3.1% to 2.1%. Use simple annualization (`periodReturn × 365/days`) for cross-scenario comparison, and always show the actual period return as the primary metric. Never report compound annualized returns from sub-year data as real expected returns.
